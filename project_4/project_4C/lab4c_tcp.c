...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (poll_ret < 0) {
            error("lab4c_tcp error: failed to poll from socket file descriptor", 2);
        }
        else if (poll_ret > 0) {
...
void client_connect() {
    ...
    if (sockfd < 0) {
        error("lab4c_tcp error: failed to create socket", 2);
    }

    // convert hostname to IP address
    server = gethostbyname(host_arg);
    if (server == NULL) {
        error("lab4c_tcp error: invalid hostname", 1);
    }
    ...
    if (connect(sockfd, (struct sockaddr *) &server_addr, sizeof(server_addr)) < 0) {
        error("lab4c_tcp error: failed to connect to socket", 2);
    }
}
...
void register_log_file(const char *filename) {
    ...
    if (!log_file_ptr) {
        error("lab4c_tcp: failed to create or open log file", 1);
    }
}
    ...
    if (strcmp(command, "SCALE=F") == 0) {
        ...
        for (unsigned int i=0; i<strlen(new_period_arg); i++) {
            if (!isdigit(new_period_arg[i])) {
                ...
                shutdown_procedure();
                error("lab4c_tcp: invalid argument to the command 'PERIOD='", 2);
            }
        }
...
void read_stdin() {
    ...
    if (read_ret < 0) {
        error("lab4c_tcp error: failed to read from socket", 2);
    }
    ...
    for (int i=0; i<read_ret; i++) {
        if (buf[i] == newline) {
            ...
            if (command_buf_place > 1023) {
                shutdown_procedure();
                error("lab4c_tcp error: recieved socket command is too large", 2);
            }
        }
    }
}
...
void print_temp_entry() {
    ...
    if (chars_written < 0) {
        error("lab4c_tcp error: sprintf() failed when creating temp report", 2);
    }
...
void write_to_sockfd(char* buf, int chars_to_write) {
    if (chars_to_write < 0) {
        error("lab4c_tcp error: call to sprintf() failed while printing to sock file descriptor", 2);
    }

    // write message to sockfd
    ssize_t write_ret = write(sockfd, buf, chars_to_write);
    if (write_ret < 0) {
        error("lab4c_tcp error: failed to write to sock file descriptor", 2);
    }
}
...
void get_current_time() {
    ...
    if (time_buf_bytes < 0) {
        error("lab4c_tcp error: sprintf() failed while retrieving current time", 2);
    }
...
float get_temp_reading() {
    ...
    if (scale_arg == C) {
        ...
        error("lab4c_tcp error: argument to '--scale' must be either 'C' or 'F'", 2);
        return -1;
    }
}
...
void shutdown_procedure() {
    ...
    // close the socket
    if (close(sockfd) < 0) {
        error("lab4c_tcp error: failed to close socket", 2);
    }
}
...
void process_args(int argc, char* argv[]) {
    ...
    host_arg = NULL;
    int id_passed = 0;
    char* usage_statement = "Usage: lab4c_tcp --id=9-digit-num --host=name_or_address --log=filename \
[--period=N] [--scale=(C|F)] port_num";
        ...
        switch(returned_option) {
            case p:
                for (unsigned int i=0; i<strlen(optarg); i++) {
                    if (!isdigit(optarg[i])) {
                        error("lab4c_tcp: argument to '--period' must consist of all digits", 1);
                    }
                }
            ...
            case s:
                if (strlen(optarg) != 1) {
                    fprintf(stderr, "lab4c_tcp: argument to '--scale' must be 'C' or 'F'\n");
                    fprintf(stderr, "%s\n", usage_statement);
                    exit(1);
                }
                scale_arg = optarg[0];
            ...
            case id:
                ...
                for (unsigned int i=0; i<strlen(optarg); i++) {
                    if (!isdigit(optarg[i])) {
                        error("lab4c_tcp error: argument to '--id' must consist of all digits", 1);
                    }
                }
                id_arg = atoi(optarg);
                break;
    ...
    // get the port number
    if (optind >= argc) {
        fprintf(stderr, "lab4c_tcp error: no port_number passed\n");
        fprintf(stderr, "%s\n", usage_statement);
        exit(1);
    }
    else {
    ...
    if (scale_arg != F && scale_arg != C) {
        fprintf(stderr, "lab4c_tcp: argument to '--period' must be 'C' or 'F'\n");
        fprintf(stderr, "%s\n", usage_statement);
        exit(1);
    }
    ...
    if (!id_passed) {
        fprintf(stderr, "lab4c_tcp: no --id=id_num argument passed\n");
        fprintf(stderr, "%s\n", usage_statement);
        exit(1);
    }
    else if (id_arg < 100000000 || id_arg > 999999999) {
        fprintf(stderr, "lab4c_tcp: id must be a 9-digit number\n");
        fprintf(stderr, "%s\n", usage_statement);
        exit(1);
    }
    ...
    if (!host_arg) {
        fprintf(stderr, "lab4c_tcp: no --host=name_or_address argument passed\n");
        fprintf(stderr, "%s\n", usage_statement);
        exit(1);
    }
    ...
    if (!log_file_ptr) {
        fprintf(stderr, "lab4c_tcp error: no --log=filename argument passed\n");
        fprintf(stderr, "%s\n", usage_statement);
        exit(1);
    }
}